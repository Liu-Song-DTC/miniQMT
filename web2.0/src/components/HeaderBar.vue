  ...
  <header class="bg-white/80 backdrop-blur-md border-b border-slate-200/60 sticky top-0 z-40">
    ...
    <div class="px-4 md:px-6 pb-2 flex items-center justify-between gap-1.5 flex-wrap text-[11px]">
      ...
      <div class="flex items-center gap-1 flex-wrap">
        ...
        <button @click="toggleStopProfit" :disabled="stopProfitLoading" :class="['px-2 py-1 rounded-md font-semibold transition-colors', stopProfitEnabled ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600']">{{ stopProfitLoading ? '...' : '止盈 ' + (stopProfitEnabled ? 'OFF' : 'ON') }}</button>
