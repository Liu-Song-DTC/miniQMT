...
- **成交确认落账** — 动态止盈止损、补仓和网格实盘委托均等成交确认后写 `trade_records`
- **智能网格交易** — 自动低吸高抛，支持回调触发、多档位、风险分级和 FIFO 真实盈亏账本
- **网格状态口径统一** — Web 悬停卡片使用真实盈亏快照，中心价偏离展示网格中心漂移而非市价偏离
- **自动买入模块** — 独立进程从候选池筛选标的，复用 Web 买入 API 下单
- **行情源健康评分** — 内存观察 xtdata/Mootdx 成功率、延迟、新鲜度和数据质量
- **数据库维护与日志轮转** — 非交易时段清理追加型历史数据并轮转 XtQuantManager 日志
