...
| `MARKET_HEALTH_OBSERVE_ONLY` | `True` | 健康评分仅观察，不拦截交易信号 |
...
当前回归配置包含 29 个测试组（含 `fast` 快速子集）、89 个模块引用、64 个唯一测试模块；最近一次 `--all` 实测运行 28 个非 fast 组、65 个模块引用、961 个用例，100% 通过。
...
第一阶段为轻量内存版，不落库、重启即清空；默认 `MARKET_HEALTH_OBSERVE_ONLY = True`，只记录评分不拦截交易。
